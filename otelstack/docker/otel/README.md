...
Open `otel-collector-config.yaml` and we do not need to update line 45 because Vagrantfile will change __CHECKMK_ENDPOINT__ to ${GATEWAY}:4317

```yaml
exporters:
  otlp/checkmk:
    endpoint: "__CHECKMK_ENDPOINT__"  # No need to change
```
...
1. Go to **Setup > Telemetry > OpenTelemetry Collector**
2. Add a new rule:
   - GRPC-based receiver : `[x] Endpoint`
   - IP address or hostname : `put checkmk site IP Address`
   - Port: `4317`
   - leave everything else as they are
3. **Save**
4. Click **Activate Changes** (very important!)

Verify Checkmk is listening from CheckMK site SSH console:
```bash
...
In the created otel Vagrant box :
...
```
Otherwise, check docker-compose.yaml node-exporter node, especially for :
- networks:
    - otel-network 
  Make sure it is the same network with prometheus network
- ports:
    - "9100:9100"
...
2. Configure:
   ...
   - **OpenTelemetry data**: Select "Enabled"
     - **Resource attribute for hostname lookup**: `service.name`
3. **Save**
4. Click **Run service discovery** → **Full scan**
    When successful, you should see : 
    [otel_metrics]: Success
    in the GUI. Then, Accept all
